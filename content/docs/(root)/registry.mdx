...
The **pdfcn** registry ships Takumi PDF and Forme PDF components, blocks, and themes in the [shadcn registry format](https://ui.shadcn.com/docs/registry). Register the namespace once in `components.json`, then pull items by name with the CLI.
...
Items are namespaced by base: `takumi/` for Takumi PDF and `forme/` for Forme PDF.
