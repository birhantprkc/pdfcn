...
A React project that can run either [Takumi PDF](https://takumi.kane.tw/docs/pdf) or [Forme](https://docs.formepdf.com).

## Installation

Run the following command to add a Takumi PDF component:
...
```tsx
import { Document, Page } from "@/components/pdf/takumi-primitives";
import { PdfcnThemeProvider } from "@/components/pdf/pdfcn-theme-context";
...
```bash
npx shadcn@latest add @pdfcn/forme/theme-minimal
```
